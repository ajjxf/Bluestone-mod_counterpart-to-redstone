 ...
 * {@code isSignalSource()} and is not a coloured component is treated as neutral, so sources
...
public final class ColorAttribution {
    ...
    public static boolean isNeutralSource(BlockState state) {
        return state.isSignalSource() && !isColored(state) && !isPurpleColored(state) && !isConverter(state);
    }
}
